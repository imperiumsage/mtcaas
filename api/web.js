...
app.get('/bankrate',function(req,res){
	...
	var query = connection.query(sql, function(err, rows, fields) {
		...
		GA.initialize('UA-42619372-2', 'trackathon.com', function () {
		  GA.trackPage('Inter Bank Rates', '/bankrate', function (err, resp) {
...
app.get('/bestrate/:amount_in_usd',function(req,res){
	...
	var query = connection.query(sql, function(err, rows, fields) {
		var GA = require("googalytics");
		GA.initialize('UA-42619372-2', 'trackathon.com', function () {
		  GA.trackPage('Money Transfer Rate', '/bestrate/'+amount_in_usd, function (err, resp) {
		    if (!err, resp.statusCode === 200) {
		      console.log('it worked!');
		    }
		  });
